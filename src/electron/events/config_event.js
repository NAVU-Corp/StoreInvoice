...
ipcMain.on(ConfigEvent.GET_ALL_CONFIGS, (event, filter) => {
  configService.getConfigs(filter)
    .then((configs) => event.reply(ConfigEvent.RESULT_GET_ALL_CONFIGS, {
      result: 1,
      content: {
        configs,
      }
...
ipcMain.on(ConfigEvent.GET_ONE_INVOICE, (event, { id }) => {
  configService.getConfigById(id)
    .then((config) => event.reply(ConfigEvent.RESULT_GET_ONE_CONFIG, {
      result: 1,
      content: {
        config,
      }
...
ipcMain.on(ConfigEvent.INSERT_ONE_CONFIG, (event, config) => {
  ...
  configService.createConfig(config, {
    ...
    .then((data) => event.reply(ConfigEvent.RESULT_INSERT_ONE_CONFIG, {
      result: 1,
    }))
...
ipcMain.on(ConfigEvent.UPDATE_ONE_CONFIG, (event, config) => {
  ...
  configService.updateConfig(config, {
    ...
    .then((data) => event.reply(ConfigEvent.RESULT_UPDATE_ONE_CONFIG, {
      result: 1,
    }))
...
ipcMain.on(ConfigEvent.DELETE_ONE_CONFIG, (event, { id }) => {
  ...
  configService.deleteConfig(id, {
    ...
    .then((data) => event.reply(ConfigEvent.RESULT_DELETE_ONE_CONFIG, {
      result: 1,
      content: {
        deleteid: id,
      }
